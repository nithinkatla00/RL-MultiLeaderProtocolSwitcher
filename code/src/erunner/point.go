...
import (
	...
	"github.com/nithinkatla00/RL-MultiLeaderProtocolSwitcher/erunner/config"
	"github.com/nithinkatla00/RL-MultiLeaderProtocolSwitcher/erunner/discovery"
	"github.com/nithinkatla00/RL-MultiLeaderProtocolSwitcher/erunner/kube"
	"github.com/nithinkatla00/RL-MultiLeaderProtocolSwitcher/erunner/promquery"
	"gonum.org/v1/plot"
		...
		case "ReplicaSet":
			...
			defer func() {
				...
				if err != nil {
					...
					kCli.ForceDeletePods(ctx, result.Namespace, fmt.Sprintf("app=%s", result.Name))
...
func plotTimeSeries(title string, results []promquery.Result) (*plot.Plot, error) {
	// Corrected to use a single return value
	p := plot.New() 
	...
	for _, res := range results {
		...
		// Assuming res.Metric is a string that you want to use as a label
	...
	// Save the plot to a PNG file or perform further operations as needed.
	return p, nil
}
