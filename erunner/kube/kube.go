...
func NewManager(kubeConfigPath string) (*KubeManager, error) {
	...
	// Use context.TODO() for simplicity, can be replaced with a specific context if needed.
	if _, err = clientset.CoreV1().Nodes().List(context.TODO(), metav1.ListOptions{}); err != nil {
...
func (km *KubeManager) DeleteReplicaSet(namespace, rsName string) error {
	...
	err = wait.PollImmediate(5*time.Second, 3*time.Minute, func() (bool, error) {
		p, err := km.clientset.CoreV1().Pods(corev1.NamespaceDefault).List(context.TODO(), metav1.ListOptions{LabelSelector: fmt.Sprintf("app=%s", rsName)})
	...
	if err != nil {
		return fmt.Errorf("error waiting for replicaset pods deletion: %w", err)
	}
	err = wait.PollImmediate(5*time.Second, 3*time.Minute, func() (bool, error) {
		p, err := km.clientset.AppsV1().ReplicaSets(corev1.NamespaceDefault).List(context.TODO(), metav1.ListOptions{FieldSelector: fmt.Sprintf("metadata.name=%s", rsName)})
		if len(p.Items) == 0 || apierrors.IsNotFound(err) {
			return true, nil
		}
		return false, nil
	...
	if err != nil {
		return fmt.Errorf("error waiting for replicaset deletion: %w", err)
	}
	return nil
}
...
func (km *KubeManager) MonitorReplicaSet(ctx context.Context,
	...
	watcher, err := km.clientset.CoreV1().Pods(namespace).Watch(context.TODO(), opts)
	if err != nil {
		log.Fatal(err)
	}
	for {
...
func (km *KubeManager) ForceDeletePods(ctx context.Context, namespace, label string) error {
	podClient := km.clientset.CoreV1().Pods(namespace)
	deletePolicy := metav1.DeletePropagationForeground
	grace := int64(0)

	// Use the context in the DeleteCollection call
	err := podClient.DeleteCollection(
		ctx, // Pass the context as the first argument
